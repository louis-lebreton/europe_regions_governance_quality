    ...
    column(6,
      ...
      fluidRow(column(6,
                      selectInput(inputId = "Palette",
                                  ...
                                  choices = c("Dégradé bleu","Dégradé bleu inversé","Viridis","Viridis inversé","Rouge-Bleu","Bleu-Rouge","4 clusters K-Means"),
                                  selected = "Rouge-Bleu",
                                  width = "220px")),
             column(6,
...
server <- function(input, output, session){
  ...
  output$carte1 <- renderPlot(
    ...
    {if(input$Palette == "Bleu-Rouge") scale_fill_gradient2(name="Index",low = "blue", mid = "#F7F9A8", high = "brown", midpoint = mean(geo_tidy_finder()[, input$Var1],na.rm = TRUE))}+
  ...
  output$carte2 <- renderPlot(
    ...
    ggplot() +
      ...
      {if(input$Palette2 == "Bleu-Rouge") scale_fill_gradient2(name="Index",low = "blue", mid = "#F7F9A8", high = "brown", midpoint = mean(geo_tidy_finder()[, input$Var2],na.rm = TRUE))}+
      {if(input$Palette == "4 clusters K-Means") scale_discrete_manual("fill", values = c("#FFB200", "#EEACF0", "#B8F0AC","#2DB2BB"),na.value="#555555")}+
      # legende
      labs(
